...
trait NoteApi {
  ...
  class DefaultNoteApi {
    ...
    def getNoteById: Endpoint[Note] = get(basePath :: uuid) { uuid: UUID =>
      ...
      noteServiceRepository.findById(uuid).map {
        case Some(x) => Ok(x)
        case None => NotFound(filter.Errors.notFoundError(s"No note for ID $uuid"))
      }
    }
    ...
    //        store(note).map(Created)
    //        noteServiceRepository.setNote(note.id, note)
    //    }
    //
    //    // TODO Add PUT method
